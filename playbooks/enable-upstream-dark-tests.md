...
| `.dark` module dedent heuristics in `E2EFormat` can regress other upstream files | A local dedent pop heuristic can remove malformed pseudo-tests in `eapply.dark` but makes `ematch.dark` parse fail (`Unexpected tokens after expression`) across many cases. | Do not land ad-hoc dedent logic; keep `eapply.dark` gated until module-scope parsing is redesigned and validated across the full upstream corpus. | Validate with both `./run-tests --filter=eapply.dark` and `./run-tests --filter=ematch.dark`, then confirm with full `./run-tests`. |
