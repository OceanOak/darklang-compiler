...
| `eapply.dark` remains preamble-parser blocked when runner-wired | `./run-tests --filter=eapply.dark` fails during preamble build with `Char literal contains more than one grapheme cluster: 'a> (arg: ` from generic annotations like `<'a>`. | Keep `eapply.dark` out of default include logic until compiler preamble parsing supports these generic forms. | Re-check with `./run-tests --filter=eapply.dark`, then run full `./run-tests`. |
